...
describe('LoginUsecase', () => {
  ...
  it('should log in a user with correct credentials', async () => {
    ...
    const hashPassword = await new BcryptPasswordHash().hash(student.password);
    ...
    expect(
      comparePwHash.calledOnceWithExactly(student.password, hashPassword)
    ).toBeTruthy();
    expect(
      tokenStub.calledOnceWithExactly({ studentId: student.id })
    ).toBeTruthy();
  ...
  it('should throw UnauthenticatedError when password is incorrect', async () => {
    ...
    expect(
      comparePwHash.calledOnceWithExactly(input.password, hashPassword)
    ).toBeFalsy();
  });
